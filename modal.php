...
if (!empty($_POST)) {
  ...
  if ($_POST['action'] == 'infoProducto') {
    ...
    $query = mysqli_query($conexion, "SELECT codproducto, descripcion, precio, existencia FROM producto WHERE codproducto = $producto_id OR codBarra = '$producto_id'");

    $result = mysqli_num_rows($query);
    if ($result > 0) {
      $data = mysqli_fetch_assoc($query);
      $_SESSION['infoProductocodigo'] = $data['codproducto'];
...
if ($_POST['action'] == 'addProductoDetalle') {
  if (empty($_SESSION['infoProductocodigo']) || empty($_POST['cantidad'])){
    echo 'error';
  }else {
    $codproducto = $_SESSION['infoProductocodigo'];
    ...
    $impuesto = round($sub_total / 100 * $iva, 2);
  ...
  if ($_POST['action'] == 'anularFactura') {
    ...
    if ($_POST['action'] == 'changePasword') {
      if (!empty($_POST['passActual']) && !empty($_POST['passNuevo'])) {
        ...
        Swal.fire({
            title: "Éxito!",
            text: "Actualizacion de Contraseña realizada correctamente",
            icon: "success",
